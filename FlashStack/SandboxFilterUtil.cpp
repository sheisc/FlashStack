...
sandbox::bpf_dsl::ResultExpr SandboxPolicyBase::EvaluateSyscall(
  ...
  #if 1  // added by iron
  if(__NR_arch_prctl == aSysno //|| __NR_recv == aSysno
      || __NR_mremap == aSysno || __NR_munmap == aSysno
        || __NR_mmap == aSysno || __NR_recvfrom == aSysno
          || __NR_recvmsg == aSysno ){ // || __NR_open == aSysno
    return Allow();
  }
  #endif  
